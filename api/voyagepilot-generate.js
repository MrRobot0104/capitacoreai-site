...
module.exports = async (req, res) => {
  ...
  if (applyRateLimit(req, res, 'voyagepilot', 10, 60000)) return;

  try {
    // Public endpoint for shared trips — no auth but validate UUID format
    if (req.body && req.body.action === 'get_trip') {
      ...
      if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(tripId)) {
        console.error('[SECURITY] get_trip: invalid trip ID format:', tripId);
        return res.status(400).json({ error: 'Invalid trip ID format' });
      }
      const { applyRateLimit, getClientIP } = require('./_rateLimit');
      if (applyRateLimit(req, res, 'get_trip', 20, 60000)) {
        console.error('[SECURITY] get_trip: rate limit hit from', getClientIP(req));
        return;
      }
      const tripRes = await fetch(
        process.env.SUPABASE_URL + '/rest/v1/trips?id=eq.' + tripId + '&select=trip_data',
    ...
    if (action === 'start_conversation') {
      ...
      if (newBalance === -1) return res.status(402).json({ error: 'VoyagePilot requires 2 credits. You don\'t have enough.' });

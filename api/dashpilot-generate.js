...
module.exports = async (req, res) => {
  ...
  if (action === 'start_conversation') {
    if (isAdmin) return res.status(200).json({ ok: true, remaining: 9999, cost: 3 });
    const deductRes = await fetch(supabaseUrl + '/rest/v1/rpc/deduct_credits', {
      method: 'POST',
      headers: { 'apikey': serviceKey, 'Authorization': 'Bearer ' + serviceKey, 'Content-Type': 'application/json' },
      body: JSON.stringify({ user_uuid: user.id, amount: 3 }),
    });
    if (!deductRes.ok) {
    ...
    if (newBalance === -1) return res.status(402).json({ error: 'DashPilot requires 3 credits. You don\'t have enough.' });
    return res.status(200).json({ ok: true, remaining: newBalance, cost: 3 });
  }
